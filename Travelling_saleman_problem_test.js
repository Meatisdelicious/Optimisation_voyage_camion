...
function draw() {
    ...
    while (d2 - d1 > T || i-j < 2) {
        ...
        d1 = liste_ville[i].dist(liste_ville[(i + 1) % nb_ville]) + liste_ville[j].dist(liste_ville[(j + 1) % nb_ville]);
        d2 = liste_ville[i].dist(liste_ville[j]) + liste_ville[(i + 1) % nb_ville].dist(liste_ville[(j + 1) % nb_ville]);
    }
    ...
    if (probability(pathlength, newlength, T) >= random()) {
        m = liste_ville.slice(Math.min((i + 1) % nb_ville, (j + 1) % nb_ville), Math.max((i + 1) % nb_ville, (j + 1) % nb_ville)).reverse();
        l = liste_ville.slice(0, Math.min((i + 1) % nb_ville, (j + 1) % nb_ville));
        r = liste_ville.slice(Math.max((i + 1) % nb_ville, (j + 1) % nb_ville), nb_ville);
